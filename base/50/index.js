...
//密码安全强度验证�
    ...
    $('#photo_big .close').click(function () {
        ...
        $("#photo_big .big img").attr('src','images/loading.hif').css('width','32px').css('height','32px').css('top','190px');
    });
	...
	/*
	//图片加载
	$("#photo_big .big img").attr('src','images/p1big.jpg').animate({
		attr:'o',
		target:100,
		step:10,
		t:30
	...
	//IE必须把src放在load事件的后面
    temp_img.src="images/p1big.jpg";//src属性可以在后台加载这张图片到本地缓存
	 */
    ...
    $("#photo_big .big .left").click(function(){
    	...
    	var children = $("#photo dl dt img").ge(prevIndex($("photp_big .big img").attr('index'),$("#photo").first())).parentNode.parentNode;
        prev_next_img(children)
    });
    //图片下一张
    $("#photo_big .big .right").click(function(){
        $("#photo_big .big img").attr('src',$(this).attr('src'));
        var children = $("#photo dl dt img").ge(nextIndex($("photp_big .big img").attr('index'),$("#photo").first())).parentNode.parentNode;
        prev_next_img(children)
    });
