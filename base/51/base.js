...
function Base(args) {
	...
	if(typeof args =="string"){
        //CSS模拟
		if(args.indexOf(" ")!=-1){
			var elements=args.split(" ");//把节点拆开分别保存到数组里边去
			var childElements=[];		  //存放临时节点对象的数组,解决被覆盖的问题
			var node=[];				  //存放父节点用的
			for(var i=0;i<elements.length;i++){
				if(node.length==0) node.push(document);//如果没有父节点,默认把document放入
...
Base.prototype.lock = function () {
	for (var i = 0; i < this.elements.length; i ++) {
		...
		this.elements[i].style.display = 'block';
		//document.documentElement.style.overflow = 'hidden';//旧版firefox不支持documentElement,支持document.body
        //document.body.style.overflow = 'auto';//但是低版本IE不支持body
    ...
    for (var i = 0; i < this.elements.length; i ++) {
       ...
       // }
        ...
        element.timer = setInterval(function () {
			/*
			 问题一:多个动画执行了多个列队动画,我们要求不管多少个动画只执行一次列队动画;
			 问题二:多个动画变化数值相差太大,导致动画无法执行到目标值,原因是定时器提前清理掉了

			 解决一:不管多少个动画,只提供一个列队动画的机会
			 解决二:多个动画按最后一个分动画执行完毕清理即可
			 */

			//创建一个布尔值,这个布尔值可以判断多个动画是否完全执行完毕
			var flag=true;//表示都执行完毕了
            for (var i in mul) {	//i为mul对象里边的属性值;mil[i]为属性的目标值
